...
export default function ResetPassword() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) { setErrors({ general: data.error || t('Invalid or expired link','رابط غير صالح أو منتهي الصلاحية') }); return }
      setDone(true)
    } catch { setErrors({ general: t('Connection error','خطأ في الاتصال') }) }
  ...
  if (!token) return (
    <div className="pt-14 min-h-screen flex items-center justify-center px-4">
      <div className="glass p-8 rounded-2xl text-center max-w-sm">
        <span className="material-symbols-outlined text-error text-4xl mb-3 block">link_off</span>
        <p className="text-on-surface font-bold mb-4">{t('Invalid reset link','رابط إعادة التعيين غير صالح')}</p>
        <Link to="/forgot-password" className="btn-primary px-6 py-3 rounded-xl">{t('Request new link','طلب رابط جديد')}</Link>
      </div>
    </div>
  )
    ...
    <div className="pt-14 pb-24 md:pb-10 min-h-screen flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-on-surface font-display">{t('Reset Password','إعادة تعيين كلمة المرور')}</h1>
          <p className="text-on-surface-variant text-sm mt-1">{t('Enter your new password below.','أدخل كلمة المرور الجديدة أدناه.')}</p>
        </div>

        {!done ? (
          <div className="glass p-6 rounded-2xl space-y-4 animate-fade-in">
            {errors.general && (
              <div className="p-3 rounded-xl text-sm text-error text-center" style={{background:'rgba(179,38,30,0.08)', border:'1px solid rgba(179,38,30,0.2)'}}>
                {errors.general}
              </div>
            )}
            <div>
              <label className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2 block">{t('New Password','كلمة المرور الجديدة')}</label>
              <div className="relative">
                <input type={showPw?'text':'password'} className={`rasha-input pe-12 ${errors.password?'border-error':''}`}
            ...
            <div>
              <label className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2 block">{t('Confirm Password','تأكيد كلمة المرور')}</label>
              <div className="relative">
                <input type={showConfirm?'text':'password'} className={`rasha-input pe-12 ${errors.confirm?'border-error':''}`}
                ...
                <button type="button" onClick={()=>setShowConfirm(p=>!p)} className="absolute end-3 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-secondary-fixed">
          ...
          <div className="glass p-8 rounded-2xl text-center animate-fade-in space-y-4">
            ...
            <p className="text-on-surface-variant text-sm">{t('Your password has been reset successfully. Sign in with your new password.','تم إعادة تعيين كلمة مرورك. سجل الدخول بكلمة المرور الجديدة.')}</p>
